namespace Dashing.Engine {
    ...
    public class SqlEngine : IEngine {
        private readonly ISelectWriter selectWriter;

        private readonly ICountWriter countWriter;

        private readonly IUpdateWriter updateWriter;

        private readonly IBulkUpdateWriter bulkUpdateWriter;

        private readonly IInsertWriter insertWriter;

        private readonly IDeleteWriter deleteWriter;

        private readonly IBulkDeleteWriter bulkDeleteWriter;
        ...
        public T Query<T, TPrimaryKey>(IDbConnection connection, IDbTransaction transaction, TPrimaryKey id)
            ...
            using (var duration = new DependencyExecutionTimer(d => this.LogSqlCall(d, sqlQuery))) {
                var entity = connection.Query<T>(sqlQuery.Sql, sqlQuery.Parameters, transaction)
                                       .SingleOrDefault();
                if (entity != null) {
        ...
        public IEnumerable<T> Query<T, TPrimaryKey>(IDbConnection connection, IDbTransaction transaction, IEnumerable<TPrimaryKey> ids)
            ...
            using (var duration = new DependencyExecutionTimer(d => this.LogSqlCall(d, sqlQuery))) {
                return connection.Query<T>(sqlQuery.Sql, sqlQuery.Parameters, transaction)
                                 .Select(
                                     t => {
        ...
        public Page<T> QueryPaged<T>(IDbConnection connection, IDbTransaction transaction, SelectQuery<T> query)
            ...
            var totalResults = connection.Query<int>(countQuery.Sql, countQuery.Parameters, transaction)
                                         .SingleOrDefault();

            return new Page<T> {
                                   TotalResults = totalResults,
                                   Items = this.Query(connection, transaction, query)
                                               .ToArray(),
                                   Skipped = query.SkipN,
                                   Taken = query.TakeN
                               };
        ...
        public int Count<T>(IDbConnection connection, IDbTransaction transaction, SelectQuery<T> query)
            where T : class, new() {
            var countQuery = this.countWriter.GenerateCountSql(query);
            return connection.Query<int>(countQuery.Sql, countQuery.Parameters, transaction)
                             .SingleOrDefault();
        }
        ...
        private static void InsertAndSetId<T, TKey>(IDbConnection connection, IDbTransaction transaction, string sqlAndReturnId, DynamicParameters parameters, IMap<T> map, T entity)
            where T : class, new() {
            var idResult = connection.Query<TKey>(sqlAndReturnId, parameters, transaction);
        ...
        public async Task<Page<T>> QueryPagedAsync<T>(IDbConnection connection, IDbTransaction transaction, SelectQuery<T> query)
            where T : class, new() {
            var sqlQuery = this.countWriter.GenerateCountSql(query);

            using (var duration = new DependencyExecutionTimer(d => this.LogSqlCall(d, sqlQuery))) {
                var totalResults = (await connection.QueryAsync<int>(sqlQuery.Sql, sqlQuery.Parameters, transaction)).SingleOrDefault();
        ...
        public async Task<int> CountAsync<T>(IDbConnection connection, IDbTransaction transaction, SelectQuery<T> query)
            ...
            using (var duration = new DependencyExecutionTimer(d => this.LogSqlCall(d, sqlQuery))) {
                return (await connection.QueryAsync<int>(sqlQuery.Sql, sqlQuery.Parameters, transaction)).SingleOrDefault();
            }
        }
        ...
        private static async Task InsertAndSetIdAsync<T, TKey>(IDbConnection connection, IDbTransaction transaction, string sqlAndReturnId, DynamicParameters parameters, IMap<T> map, T entity)
            where T : class, new() {
            var idResult = await connection.QueryAsync<TKey>(sqlAndReturnId, parameters, transaction);
        ...
        private void LogSqlCall(TimeSpan duration, SqlWriterResult sqlQuery, [CallerMemberName] string callerMemberName = "") {
            Log.Logger?.Info($"Dashing - Called {callerMemberName}. Duration: {duration}.\nSQL: {sqlQuery.Sql}");
        }
    }
}