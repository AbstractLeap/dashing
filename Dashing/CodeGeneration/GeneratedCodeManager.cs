namespace Dashing.CodeGeneration {
    ...
    public class GeneratedCodeManager : IGeneratedCodeManager {
        ...
        public IEnumerable<T> Query<T>(SelectWriterResult result, SelectQuery<T> query, IDbTransaction transaction) {
            if (query.HasFetches()) {
                ...
                if (this.compileTimeFunctionExistsFunction(typeof(T), result.FetchTree.FetchSignature)) {
                    if (query.IsTracked) {
                        return this.Tracked(((DelegateQuery<T>)this.queryCalls[typeof(T)])(result, query, transaction.Connection, transaction));
                    }

                    return ((DelegateQuery<T>)this.queryCalls[typeof(T)])(result, query, transaction.Connection, transaction);
                }
                ...
                if (result.HasCollectionFetches) {
                    ...
                    return this.delegateQueryCreator.GetFKCollectionFunction<T>(result, false)(result, query, transaction);
                }

                if (query.IsTracked) {
                    return this.Tracked(this.delegateQueryCreator.GetTrackingNoCollectionFunction<T>(result, true)(result, query, transaction));
                }

                return this.delegateQueryCreator.GetFKNoCollectionFunction<T>(result, false)(result, query, transaction);
            }

            if (query.IsTracked) {
                return this.Tracked(((NoFetchDelegate<T>)this.noFetchTrackingCalls[typeof(T)])(transaction.Connection, result.Sql, result.Parameters, transaction));
            }

            return ((NoFetchDelegate<T>)this.noFetchFkCalls[typeof(T)])(transaction.Connection, result.Sql, result.Parameters, transaction);
        }

        public IEnumerable<T> Query<T>(SqlWriterResult result, IDbTransaction transaction, bool asTracked = false) {
            if (asTracked) {
                return this.Tracked(((NoFetchDelegate<T>)this.noFetchTrackingCalls[typeof(T)])(transaction.Connection, result.Sql, result.Parameters, transaction));
            }
        ...
        public IEnumerable<T> Query<T>(IDbTransaction transaction, string sql, dynamic parameters = null) {
            return transaction.Connection.Query<T>(sql, new DynamicParameters(parameters), transaction);
        }
