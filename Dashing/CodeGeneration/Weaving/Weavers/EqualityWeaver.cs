        ...
        public override void Weave(AssemblyDefinition assemblyDefinition, TypeDefinition typeDefinition, IEnumerable<ColumnDefinition> columnDefinitions)
        {
            ...
            if (!this.DoesNotUseObjectMethod(typeDefinition, "GetHashCode"))
            {
                ...
                var getHasValueMethodRef =
                    MakeGeneric(
                        typeDefinition.Module.ImportReference(hashCodeBackingField.FieldType.Resolve().GetMethods().Single(m => m.Name == "get_HasValue")),
                ...
                if (isGuidPk)
                {
                    ...
                    il.Add(
                        Instruction.Create(OpCodes.Call, typeDefinition.Module.ImportReference(guidTypeDef.Resolve().Methods.Single(m => m.Name == "op_Equality"))));
                ...
                il.Add(Instruction.Create(OpCodes.Ldarg_0));
                il.Add(Instruction.Create(OpCodes.Ldarg_0));
                il.Add(Instruction.Create(OpCodes.Call, typeDefinition.Module.ImportReference(typeof(object).GetMethods().Single(m => m.Name == "GetHashCode"))));
                il.Add(
                    Instruction.Create(
                        OpCodes.Newobj,
                        MakeGeneric(
                ...
                if (isGuidPk)
                {
                    ...
                    il.Add(
                        Instruction.Create(OpCodes.Callvirt, typeDefinition.Module.ImportReference(typeof(object).GetMethods().Single(m => m.Name == "GetHashCode"))));
                }
                else if (isStringPk)
                {
                    il.Add(
                ...
                else
                {
                    ...
                    il.Add(Instruction.Create(OpCodes.Ldloca_S, var1));
                    il.Add(Instruction.Create(OpCodes.Call, typeDefinition.Module.ImportReference(intTypeDef.Resolve().Methods.Single(m => m.Name == "GetHashCode"))));
            ...
            if (!this.DoesNotUseObjectMethod(typeDefinition, "Equals"))
            {
                ...
                if (isGuidPk)
                {
                    ...
                    il.Add(
                        Instruction.Create(OpCodes.Call, typeDefinition.Module.ImportReference(guidTypeDef.Resolve().Methods.Single(m => m.Name == "op_Inequality"))));
                }
