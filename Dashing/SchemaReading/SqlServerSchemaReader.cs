...
ORDER BY 
     ...
     , case 
		when COLUMNPROPERTY(OBJECT_ID('[' + col.TABLE_SCHEMA + '].[' + col.TABLE_NAME + ']'), col.COLUMN_NAME, 'IsIdentity') > 0 then 1
		when col.DATA_TYPE = 'uniqueidentifier' and CAST(ISNULL(pk.is_primary_key, 0) AS bit) = 1 and col.COLUMN_DEFAULT like '%newsequentialid%' then 1
		else 0
