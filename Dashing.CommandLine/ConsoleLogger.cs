    ...
    public class ConsoleLogger : ILogger {
        ...
        public void Trace<T>(IEnumerable<T> items, string[] columnHeaders = null) {
            ...
            if (columnHeaders == null) {
                columnHeaders = props.Select(p => p.Name)
                                     .ToArray();
            }
            ...
            for (int rowIndex = 1; rowIndex < arrValues.GetLength(0); rowIndex++) {
                for (int colIndex = 0; colIndex < arrValues.GetLength(1); colIndex++) {
                    arrValues[rowIndex, colIndex] = props.ElementAt(colIndex)
                                                         .GetValue(items.ElementAt(rowIndex - 1))
                                                         .ToString();
                }
            }
        ...
        private int[] GetMaxColumnsWidth(string[,] arrValues) {
            ...
            for (int colIndex = 0; colIndex < arrValues.GetLength(1); colIndex++) {
                for (int rowIndex = 0; rowIndex < arrValues.GetLength(0); rowIndex++) {
                    int newLength = arrValues[rowIndex, colIndex]
                        .Length;
