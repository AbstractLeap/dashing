namespace Dashing.Tools.Tests.Migration {
    ...
    public class SimpleTests {
        ...
        public void DropEntityWorks() {
            ...
            var script = migrator.GenerateSqlDiff(from.Maps, to.Maps, null, null, new string[0], out warnings, out errors);
            Assert.Equal(@"drop table [PostComments];
", script);
        }
        ...
        public void AddEntityWorks() {
            ...
            IEnumerable<string> errors;
            var script = migrator.GenerateSqlDiff(from.Maps, to.Maps, null, null, new string[0], out warnings, out errors);
            Assert.Equal(@"create table [PostComments] ([Id] int not null identity(1,1) primary key, [Content] nvarchar(255) null, [PostId] int null);
...
", script);
        ...
        public void AddPropertyWorks() {
            ...
            IEnumerable<string> errors;
            var script = migrator.GenerateSqlDiff(from.Maps, to.Maps, null, null, new string[0], out warnings, out errors);
            Assert.Equal(@"alter table [PostComments] add [Rating] int not null default (0);
", script);
        }
        ...
        public void DropPropertyWorks() {
            ...
            IEnumerable<string> errors;
            var script = migrator.GenerateSqlDiff(from.Maps, to.Maps, null, null, new string[0], out warnings, out errors);
            Assert.Equal(Regex.Replace(@"declare @OBDCommandca45edee90bb4cc68430f8540d28aa99 nvarchar(1000);
...
alter table [PostComments] drop column [Rating];
", @"@\w+\b", "@Foo"), Regex.Replace(script, @"@\w+\b", "@Foo"));
        ...
        public void AddNewRelationshipToTableWithNoData() {
            ...
            IEnumerable<string> warnings;
            IEnumerable<string> errors;
            var script = migrator.GenerateSqlDiff(from.Maps, to.Maps, answerProvider.Object, null, new string[0], out warnings, out errors);
            Assert.Equal(@"create table [Blogs] ([Id] int not null identity(1,1) primary key, [Title] nvarchar(255) null);
